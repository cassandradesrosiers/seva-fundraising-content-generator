...
// ─── GRAPHIC PREVIEW ─────────────────────────────────────────────────────────
// Font: Nunito loaded via Google Fonts (injected once into the document head)
const NUNITO_LOADED = { done: false };
function ensureNunito() {
  if (NUNITO_LOADED.done || typeof document === "undefined") return;
  NUNITO_LOADED.done = true;
...
function GraphicPreview({ content, size, format, raised: raisedAmount }) {
  ...
  // Use live raisedAmount if provided, fall back to campaign default
...
export default function SEVAGenerator() {
  ...
  // ── Live sync from southendvillageacademy.org ─────────────────────────────
  // Uses Anthropic's web_search tool (server-side fetch, no CORS issue).
  // The SEVA website pulls from Zeffy and updates every ~10 minutes.
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `Fetch southendvillageacademy.org right now and find the exact current dollar amount raised displayed on the page. The site shows a live Zeffy donation counter updated every 10 minutes. Return ONLY this JSON with no other text: {"raised": "$XXX,XXX"}`,
          }],
        }),
      });
      const data = await res.json();
      // Response contains tool_use blocks then a final text block
      ...
      if (amount) {
        setRaisedOverride(amount);
        setIsLive(true);
        setSyncTime(new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }));
      } else {
        setSyncError("Could not parse amount from website — enter manually.");
      }
    } catch (e) {
  ...
  const handleEditInCanva = () => {
    ...
    const B = CAMPAIGN.brand;
    const msg = `Please create a Canva design for SEVA with this content.\n\nFORMAT: ${sz.label} (${sz.sub})${graphicSize === "letter" ? ` — ${graphicFormat}` : ""}\n\nHEADLINE: ${graphicContent.headline}\nSUBHEADLINE: ${graphicContent.subheadline}\nSTAT: ${graphicContent.stat}\nBODY: ${graphicContent.bodyText}\nCTA: ${graphicContent.cta}\nURL: ${graphicContent.url}\nMATCH LINE: ${graphicContent.matchLine}\n\nSEVA brand colors: teal ${B.teal} (primary — hero, headers), orange ${B.orange} (CTAs, progress bar, match badge), navy ${B.navy} (footer, dark text), cream ${B.cream} (warm section backgrounds). Font: Nunito (rounded, bold). Warm, community-centered, urgent-hopeful tone. School name: South End Village Academy / SEVA. Design type: ${graphicSize === "square" ? "instagram_post" : graphicSize === "vertical" ? "your_story" : "flyer"}.`;
                      ...
                      {!canvaRequested ? (
                        ...
                        <div style={{ background:"var(--color-background-success)", border:"0.5px solid var(--color-border-success)", borderRadius:"var(--border-radius-lg)", padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"12px" }}>
                          <div style={{ display:"flex", gap:"10px", alignItems:"center" }}>
                            ...
                            <div>
                              <div style={{ fontSize:"13px", fontWeight:"700", color:"var(--color-text-success)" }}>Canva design created</div>
                              <div style={{ fontSize:"11px", color:"var(--color-text-success)", opacity:.85 }}>See the Edit in Canva link in the chat below ↓</div>
                            </div>
                          </div>
                          <button
                            onClick={downloadGraphic}
