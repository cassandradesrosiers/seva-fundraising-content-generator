...
export async function GET() {
  try {
    const message = await client.messages.create({
      ...
      messages: [{
        role: "user",
        content: `Fetch southendvillageacademy.org right now and find the exact current dollar amount raised shown on the donation counter. Return ONLY this JSON: {"raised": "$XXX,XXX"}`,
      }],
    });
